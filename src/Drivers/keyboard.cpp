...
namespace Drishti{
	...
	namespace Drivers{
		...
		KeyboardDriver::KeyboardDriver(HComms::InterruptManager* Manager, KeyboardEventHandler *Handler):InterruptHandler(0x21, Manager), DataPort(0x60), CommandPort(0x64){
		...
		void KeyboardDriver::Activate(){
			...
			Types::uint8_t status = (DataPort.Read() | 1) & ~0x10;
			CommandPort.Write(0x60); //set/change state
			DataPort.Write(status);
			DataPort.Write(0xF4);
		}

		Types::uint32_t KeyboardDriver::HandleInterrupt(Types::uint32_t esp){
			Types::uint8_t key = DataPort.Read();
